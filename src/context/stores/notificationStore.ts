...
export const useNotificationStore = create<NotificationStore>((set, get) => ({
  ...
  loadNotifications: async () => {
    ...
    try {
      ...
      handleSilentError(error, 'Load Notifications');
      set({ notifications: [], unreadCount: 0, isLoading: false });
    }
  },
  ...
  markAsRead: async (notificationId) => {
    ...
    try {
      ...
      handleSilentError(error, 'Mark Notification Read');
    }
  },
  ...
  markAllAsRead: async () => {
    ...
    if (userId) {
      try {
        ...
        handleSilentError(error, 'Mark All Notifications Read');
      }
    }
  },
  ...
  deleteNotification: async (notificationId) => {
    ...
    try {
      ...
      handleSilentError(error, 'Delete Notification');
    }
  },
  ...
  clearAll: async () => {
    ...
    if (userId) {
      try {
        ...
        handleSilentError(error, 'Clear All Notifications');
      }
    }
  },
    ...
    import('react-native-toast-message').then(({ default: Toast }) => {
      Toast.show({
        ...
        onPress: () => {
          ...
          // This should ideally use the same logic as NotificationCard
          Toast.hide();
        }
      });
    });
